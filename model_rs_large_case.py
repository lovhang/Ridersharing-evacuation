...
N_2 = np.array(n_2) # flexible driver
N_3 = np.array(n_3) # passenger
...
for i in range(len(passenger_count)):
    dm[i+1] = max(1,passenger_count[i])

cv = np.array([0.0 for i in N])
for i in range(0, num):
    cv[i] = max(dm[i], random.randint(3,5))
...
# Tolerance
LDP = np.array([T_end for i in N])  # latest departure time, the time location is affected
EDP = np.array([0 for i in N])  # Earliest departure time
...
#e_param.to_csv('case/case1_param_1011.csv')
...
#    e_param = pd.DataFrame(
#        {'id': N, 'demand': dm, 'capacity': cv, 'earlist_departure': EDP, 'lateste_departure': LDP, 'detour': DT})
#    with open('case/param1011_dm_cv_EDP_LDP_DT.pkl', 'rb') as f:
#        dm, cv, EDP, LDP, DT = pickle.load(f)
#        f.close()
...
class dynapro:
    def __init__(self, ratio):
        print("initiate")
        self.ratio = ratio # different heuristic ratio
        self.solutionpoll = []
        self.Ncan = []
        self.candidateState = []
        self.ite = 0
    ...
    def action(self, inputstate: state, des: int, it_num: int):
        S = inputstate.S.copy()
        R = inputstate.R.copy()
        T = inputstate.T.copy()
                ...
                for i in range(len(tt[l][des])):  # if i is the destination of evacuees
                    ...
                    for j in range(0, len(R)):
                        ...
                        for k in range(len(tt[des][N_D[j]])):
                            ...
                            if tempt + min_flex_interval < dt[j] and tempt + min_flex_interval <= ScenTimes[
                                k] and tempt + min_flex_interval <= dt[0]:
                                tempf = True
    ...
    def eliminate(self, input):
        ...
        for i in range(len(inputstate)):
            ...
            if (de == False):
                uniqueState.append(state1)
        return uniqueState
    def output_trip_attribute(self):
        trip_length=[0 for i in range(0,20)]
        for item in self.candidateState:
            trip_length[item.e_num] +=1
        return trip_length
class dymaster:
...
def main(triger: int):
    ...
    # ============run model on dynamic programming==========
        ...
        start = time.time()
        candidate_state = []
        for i in range(0, len(heuristic_ratio)):
            ts = dynapro(heuristic_ratio[i])
            ts.rundp(ts.solutionpoll)
            candidate_state.append(ts.candidateState)
            print('========solution_arrtibutes_for_iteration '+str(i)+' ========')
            print(ts.output_trip_attribute())
            print("generate {state_num} states".format(state_num = ts.state_num))
        end = time.time()
        dyma = dymaster(candidate_state)
        dyma.masterprob()
        dyma.solve()
        #dyma.plotmap()
        #dyma.plotaltroute()
...
if __name__ == "__main__":
    print('test')
    #load_current_network_para()
    #save_current_network_para()
    main(1)
    #vis = visualize()
    #vis.shownetwork()
    #vis.showroutes()
